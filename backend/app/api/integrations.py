...
def send_slack_notification(webhook_url: str, session: dict, score: dict):
    ...
    try:
        ...
        msg = {
            "text": f"LaunchGuard analysis complete: *{title}*",
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn",
                 "text": f"*LaunchGuard — {title}*\nReadiness Score: *{s}/100 (Grade {grade})* · {highs} high risks identified"}},
                {"type": "section", "fields": [
    ...
    for item in must_items[:10]:
        try:
            payload = {
                "fields": {
                    "project": {"key": project},
                    "summary": f"[LaunchGuard] {item.get('item', 'Checklist item')}"[:255],
...
def post_github_pr_comment(token: str, repo: str, pr: int,
    ...
    try:
        ...
        grade = score.get("grade", "?")
        lines = [
            f"## LaunchGuard Release Readiness — {feature_title}",
            "",
...
def create_linear_issues(linear_token: str, team_id: str, nav: dict, session_id: str) -> list:
    ...
    for item in must_items[:10]:
        try:
            ...
            """
            variables = {
                "title": f"[LaunchGuard] {item.get('item', 'Checklist item')[:200]}",
                "description": (
...
def send_custom_webhook(webhook_url: str, webhook_secret: str,
    ...
    try:
        ...
        if webhook_secret:
            ...
            headers["X-LaunchGuard-Signature"] = f"sha256={sig}"
...
async def export_to_confluence(session_id: str, body: ConfluenceExport, email: str = Depends(verify_token)):
    ...
    content += "</tbody></table>"
    content += f"<p><em>Generated by LaunchGuard · Session {session_id[:8]}</em></p>"

    payload = {
        "type": "page",
        "title": f"LaunchGuard: {meta.get('feature_title', '')}",
...
async def export_to_notion(session_id: str, body: NotionExport, email: str = Depends(verify_token)):
    ...
    blocks = [
        {"object": "block", "type": "heading_1",
         "heading_1": {"rich_text": [{"type": "text", "text": {"content": f"LaunchGuard: {meta.get('feature_title', '')}"}}]}},
    ...
    payload = {
        ...
        "properties": {"title": {"title": [{"type": "text", "text": {"content": f"LaunchGuard: {meta.get('feature_title', '')}"}}]}},
        "children": blocks
    }
    try:
        r = http_requests.post(
